export const useMogiData = () => {
  ...
  async function loadMogiData(season = 4, forceRefresh = false) {
    ...
    console.log(
      `Fetching season ${currentSeason} mogi data from API${forceRefresh ? ' (forced refresh)' : ''}`
    )
    mogiData.value = []
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        console.log(
          `Successfully cached ${data.length} mogis for season ${currentSeason}`
        )
      }
      mogiData.value = data || []
    } catch (e) {
      ...
      if (fallbackData && fallbackData.length > 0) {
        console.log(
          `API failed, using expired cache for season ${currentSeason} mogi data`
        )
  ...
  function calculateStats(mogis) {
    ...
    const averageDisconnections
      = mogis.reduce((sum, mogi) => sum + mogi.disconnections, 0) / mogis.length
    const maxDisconnections = Math.max(
      ...mogis.map(mogi => mogi.disconnections)
    )

    // Calculate subs
    const averageSubs
      = mogis.reduce((sum, mogi) => sum + mogi.subs, 0) / mogis.length
    ...
    mogis.forEach((mogi) => {
      const formatName
        = keyToFormat[mogi.format] || `${mogi.format}v${mogi.format}`
      formatsDict[formatName] = (formatsDict[formatName] || 0) + 1
    })

    // Calculate average players per mogi using mogi.players
    const averagePlayersPerMogi
      = mogis.reduce(
        (sum, mogi) =>
          sum + (Array.isArray(mogi.players) ? mogi.players.length : 0),
        0
      ) / mogis.length
