export const useGuildData = () => {
  ...
  const latestRequestId = useState("latestGuildDataRequestId", () => 0);
  ...
  async function loadGuildData(season = 4, forceRefresh = false) {
    ...
    console.log(
      `Fetching season ${currentSeason} guild data from API${forceRefresh ? " (forced refresh)" : ""}`,
    );
    guildData.value = [];
    ...
    try {
      ...
      if (processedData && processedData.length > 0) {
        ...
        console.log(
          `Successfully cached ${processedData.length} guilds for season ${currentSeason}`,
        );
      }
